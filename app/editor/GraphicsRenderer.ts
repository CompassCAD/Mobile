...
import { Arc, BoundBox, Circle, Component, componentTypes, Label, Line, Measure, Picture, Point, Polygon, Rectangle, Shape } from "./ComponentHandler";
...
export class GraphicsRenderer {
    ...
    start() {
        ...
        const context = (this.displayRef as HTMLCanvasElement)?.getContext('2d');
        if (!context) {
    ...
    update() {
        const htmlElement = this.displayRef as HTMLCanvasElement;
        this.offsetX = htmlElement?.offsetLeft ?? 0;
        this.offsetY = htmlElement?.offsetTop ?? 0;
